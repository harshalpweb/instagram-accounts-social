"""Publish only the founder-approved Sep23 batch; never scan the historical queue."""
...
def validate(release):
 ...
 for key,items in release['accounts'].items():
  ...
  for p in items:
   assert len(p['assets'])==(6 if p['type']=='CAROUSEL' else 1)
...
def save(state,commit=False):
 ...
 if commit:
  ...
  if staged:
   subprocess.run(['git','commit','-m','Record Sep23 batch publication state','--',rel],cwd=ROOT,check=True,stdout=subprocess.DEVNULL)
...
def verify_post(account,media_id,piece):
 ...
 if piece['type']=='CAROUSEL':
  ...
  assert len(children)==6 and all(c.get('media_type')=='VIDEO' for c in children),'Not a six-video carousel'
...
def main():
 ...
 assert os.environ.get('GITHUB_REF_NAME')=='batch/2026-09-23-research-led','Dedicated release branch required'
 state=json.loads(STATE.read_text()) if STATE.exists() else {'batch':'2026-09-23-research-led','posts':{}}
 accounts={};last={}
   ...
   if not receipt.get('container_id'):
    if piece['type']=='REELS':
     ...
     for asset in piece['assets'][len(children):]:
      children.append(post(account,account['id']+'/media',{'media_type':'VIDEO','video_url':raw+'/'+asset['path'],'is_carousel_item':'true'})['id']);save(state)
