# Hype Tingle — topic bank + anti-repetition ledger

Strategy: `income-engine/docs/hype_tingles-strategy.md` (formats F1-F5,
cast, cadence rules). This file is the working ledger so 3 posts/day
never repeats a combination. Extend it, don't reinvent it.

## Cast (fixed comic engines)
- The Cat — cold, minimal, devastating. Always the payoff, never first.
- The Dog — catastrophically positive. Wrong but pure.
- The Pigeon — unearned, unshakable confidence.
- The Goldfish — forgets mid-sentence. Accidental zen master.
- The Owl — condescending academic. Cites nothing.
- The Raccoon — chaotic hustler. Legally ambiguous advice.
- The Sloth — anti-hustle. Use sparingly (pacing joke).

## Topic wheel (evergreen human behaviors)
dating, gym, money, sleep, phones, work meetings, family WhatsApp
groups, diets, procrastination, small talk, traffic, online shopping,
gaming, "I'll start Monday", overthinking, texting habits, birthdays,
weddings, roommates, bosses, alarms/snooze, gossip, new year
resolutions, "seen" messages, salary day

## Used combinations (append every queued post)
| date | post id | format | topic | cast used |
|---|---|---|---|---|
| 2026-08-30 | 2026-08-30-animal-reviews-01 | F2 one-star-files | being a human | dog, pigeon, goldfish, raccoon, cat |
| 2026-08-30 | 2026-08-30-pigeon-confidence-01 | F3 masterclass | confidence/overthinking | pigeon |
| 2026-08-30 | 2026-08-30-animal-panel-ep01 | F1 panel (EP 01) | life advice | dog, owl, goldfish, raccoon, cat |
| 2026-08-30 | 2026-08-30-breaking-snooze-01 | F5 breaking news (REEL) | alarms/snooze | dog, owl, raccoon, cat |
| 2026-08-31 | 2026-08-31-operation-monday-story | F4 group chat (REEL, 2min story) | "I'll start Monday" / gym | full cast |
| 2026-08-31 | 2026-08-31-morning-routine-judged | NEW: judged-routine (REEL, 3D character) | 5AM morning routines | dog, pigeon, owl, raccoon, cat |
| 2026-09-01 | 2026-09-01-one-more-episode | NEW: pov-night (REEL, 3D character) | bingeing / one more episode | owl, goldfish, cat |
| 2026-09-02 | 2026-09-02-meeting-email | F5 breaking news (REEL, 3D character) | work meetings | dog, owl, raccoon, cat |
| 2026-09-01 | 2026-09-01-dating-app-reviews | F2 carousel-one-star-files | dating apps | dog, pigeon, owl, raccoon, cat |
| 2026-09-01 | 2026-09-01-raccoon-deals-masterclass | F3 carousel-masterclass (MC 02) | online shopping / deal hunting | raccoon |
| 2026-09-02 | 2026-09-02-family-group-reviews | F2 carousel-one-star-files | family WhatsApp groups | dog, goldfish, owl, pigeon, cat |
| 2026-09-02 | 2026-09-02-goldfish-birthday-masterclass | F3 carousel-masterclass (MC 03) | birthdays / forgetting wishes | goldfish |
| 2026-09-03 | 2026-09-03-diet-reviews | F2 carousel-one-star-files | diets | dog, owl, pigeon, goldfish, cat |
| 2026-09-03 | 2026-09-03-owl-smart-masterclass | F3 carousel-masterclass (MC 04) | sounding smart / meeting-speak | owl |
| 2026-09-04 | 2026-09-04-animal-panel-ep02 | F1 panel (EP 02) | red flags | dog, pigeon, owl, goldfish, cat |
| 2026-09-05 | 2026-09-04-drop-day-reviews | F2 carousel-one-star-files (deferred from 09-04 13:00 to Sat 11:30 carousel slot, cadence-ratio fix 2026-09-04; id unchanged) | drop day / limited-edition queues (trend) | dog, pigeon, raccoon, sloth, cat |
| 2026-09-04 | 2026-09-04-pre-coffee-alert | pov-morning (REEL, 2D stickman) | talking before coffee (trend: Eeffoc) | dog, owl, cat |
| 2026-09-05 | 2026-09-05-hold-and-judge-left-on-seen | NEW: reel-hold-and-judge (REEL, 2D cat rig, auto-play card carousel; moved 17:00 -> 16:30 standing Reel slot 2026-09-04 so the day stays 1 carousel + 2 Reels, new-format debut) | left on seen / texting habits | cat |
| 2026-09-04 | 2026-09-04-salary-day-breaking | F5 reel-breaking-news (REEL, 2D stickman; 16:00 fill slot for the 2-Reel ratio) | salary day / rich for 48 hours (topic wheel) | dog, raccoon, cat |
| 2026-09-10 | 2026-09-10-elevator-small-talk | pov-elevator (REEL, 2D stickman x2; emergency same-day build, founder-direct live publish ~02:05 IST, not a standing slot; scene set: elevator car + floor indicator, doorway collision gag; ratchet: lanyard follow-through) | small talk / 12 floors with a coworker (topic wheel; trend search unavailable this session) | dog, owl, cat |
| 2026-09-12 | 2026-09-12-roommate-reviews | carousel-one-star-files | roommates | dog, pigeon, goldfish, owl, cat (payoff) |
| 2026-09-12 | 2026-09-12-savings-account-judged | reel-hold-and-judge (cat-consolidation batch, founder-direct 2-hour deadline; engine reused verbatim from 2026-09-05-hold-and-judge-left-on-seen per meme-worthy-character-reels skill, hero prop swapped phone -> piggy bank so the 7-day scene/prop rule is satisfied on a fresh prop; ratchet: hook-frame stare, baseline eyelid 0.14 -> 0.20) | money / savings-account excuses (topic wheel: "money", unused before today) | cat (sole character; no human stick-figures) |
| 2026-09-12 | 2026-09-12-silent-treatment-masterclass | carousel-solo-panel (NEW: masterclass mechanic re-cast as a single-speaker cat monologue, same "speaker" visual role as THE PANEL EP01/EP02, so the cat is on-screen every slide instead of only the payoff) | silent treatment / guilt-tripping without a word | cat (sole character, all 5 content slides) |

## Queued promises to keep
- EP 01's "EP 02 drops tomorrow" promise: fulfilled by
  2026-09-04-animal-panel-ep02 (late vs. the on-air "tomorrow"; EP 02's
  own closer says only "EP 03 is loading", no hard date promised).
  Queue Panel EP 03 within a few days (topic suggestion: "Money advice"
  or "What would you ban?").
- Reviews closer says "new reviews daily" — keep F2 in rotation.
