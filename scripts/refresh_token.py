...
def main() -> None:
    ...
    # This repo holds multiple Instagram accounts' tokens under distinct
    # secret names (e.g. IG_ACCESS_TOKEN_HYPE_TINGLES) — each account's
    # workflow sets this so the refreshed token writes back to its own
    # secret, not a shared "IG_ACCESS_TOKEN". Defaults to the single-account
    # name for backward compatibility.
    secret_name = os.environ.get("IG_ACCESS_TOKEN_SECRET_NAME", "IG_ACCESS_TOKEN")
    ig_common.register_secret(current_token, admin_token, os.environ.get("META_APP_SECRET"))

    mode = ig_common.auth_mode()
    print(f"Refreshing {secret_name} (IG_AUTH_MODE={mode})")

    new_token = ig_common.refresh_token(current_token, mode)
    ig_common.register_secret(new_token)
    update_github_secret(repo, admin_token, secret_name, new_token)
    print(f"{secret_name} refreshed and updated successfully.")
