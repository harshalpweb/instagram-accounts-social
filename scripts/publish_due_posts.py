...
# content into content/failed/ (this silently killed 6 queued TrendGiri posts
# in trend-signals-social during the 2026-08-30..09-01 code=4 outage; all four
# accounts share the same Meta app, so the same blast radius applies here).
# So a rate-limit error aborts the whole run: no attempt increment, no further
# posts tried, run still red.
...
# side by side, each with its own content/queue|posted|failed. Unset/empty
# defaults to the repo root itself (single-account layout).
_REPO_ROOT = Path(__file__).resolve().parent.parent
ROOT = (_REPO_ROOT / os.environ.get("ACCOUNT_DIR", "")).resolve()
